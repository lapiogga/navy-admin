...
- [x] 01-04-PLAN.md (W4, depends: 01-01, 01-02, 01-03) -- 공통게시판(설정/카테고리/게시글/댓글/첨부파일/관리자설정/사용자설정/부대설정) 8개 서브도메인
**UI hint**: yes
...
**Plans**: 2 plans
Plans:
- [x] 02-01-PLAN.md (W1) -- 공지사항 feature slice(타입/훅/컴포넌트/MSW) + 대시보드 보강 + 로그인/로그아웃 메시지
- [x] 02-02-PLAN.md (W1) -- useSessionCheck Idle 이중 타이머 리팩토링 + SessionWarningModal + RequireAuth 연동
**UI hint**: yes
...
**Plans**: 10 plans (1 wave original + 1 wave gap closure)
Plans:
- [x] 03-01-PLAN.md (W1) -- 04_인증서발급신청체계 (신청/승인/등록대장 + 공통 재사용)
- [x] 03-02-PLAN.md (W1) -- 05_행정규칙포탈체계 (현행규정/예규/지시문서 + 즐겨찾기)
...
Plans:
- [x] 05-01-PLAN.md (W1) -- SYS07 군사자료관리체계 (보안등급 Tag + CRUD + 일괄등록 Upload + 대출 Steps + 통계 차트 4종 + 해기단자료)
- [x] 05-02-PLAN.md (W1) -- SYS10 주말버스 Part 1 (SeatGrid 좌석선택 + 예약 + 승차권 + 배차 + 시간관리 + 현황 + 게시판)
- [x] 05-03-PLAN.md (W2, depends: 05-02) -- SYS10 주말버스 Part 2 (대기자 자동/수동배정 + 위규자 제재 + 타군 전용 로그인/회원등록/관리)
- [x] 05-04-PLAN.md (W1) -- SYS18 직무기술서 Part 1 (5단계 Steps 폼 + Form.List 비율검증 + Tabs 3종 + 결재 Steps + 조직진단)
- [x] 05-05-PLAN.md (W2, depends: 05-04) -- SYS18 직무기술서 Part 2 (관리자 조회/검토/통계 차트 3종 + 표준업무시간 + 게시판/코드/권한 재사용)
**UI hint**: yes
...
- [x] 06-03-PLAN.md (W2, depends: 06-02) -- SYS01 초과근무 Part 2 (최대인정시간Tabs+Calendar근무시간/공휴일+당직개소+개인설정+게시판+관리자+라우터22개)
**UI hint**: yes
...
Plans:
- [x] 07-01-PLAN.md (W1) -- SYS03 기준정보관리4종 + 메인화면(Gauge+Bar+Bar) + 과제검색 + MSW
- [x] 07-02-PLAN.md (W1) -- SYS15 비밀/매체/보안자재 3종(SecretMediaPage type prop) + 예고문 + 인계/인수 + MSW
- [x] 07-03-PLAN.md (W2, dep:07-01) -- SYS03 연간과제관리6종(과제등록/실적입력/승인Steps/평가/추진진도율Bar)
- [x] 07-04-PLAN.md (W2, dep:07-02) -- SYS15 보안일일결산6종(Checkbox체크리스트/Calendar메인/보안수준평가/결재) 
- [x] 07-05-PLAN.md (W3, dep:07-03) -- SYS03 평가결과2종(Bar+Progress) + 게시판3(lazy) + 라우터완성
