...
**Phase 2 추가 규칙:**
- 로그인 페이지 타이틀 (`Typography.Title level={3}`): 24px semibold, 하단 여백 32px
- 로그인 부제 (`p.text-gray-500`): 14px regular, color `#8c8c8c`
- 헤더 시스템명 (`Text strong`): 18px semibold, color `#ffffff`
- 헤더 사용자 정보 (`span`): 14px regular, color `#ffffff`
- 공지사항 배너 제목: 14px semibold (antd List.Item.Meta title)
- 공지사항 배너 날짜: 12px regular, color `#8c8c8c` (antd 예외 허용)
- 서브시스템 카드 타이틀 (`Title level={5}`): 16px semibold
- 서브시스템 카드 부제 (`Text type="secondary"`): 12px regular, color `#8c8c8c`
- 세션 만료 Modal 카운트다운 숫자: 20px semibold, color `#ff4d4f` (colorError)
...
MainPortalLayout
  Header (height=64px, background=#001529)
    Left: "해군 행정포탈" 텍스트 (18px semibold, white)
...
div.flex.items-center.justify-center.min-h-screen (background=#F0F2F5)
  Card width=400px, shadow-lg
    div.text-center.mb-8
      Title level={3}: "해군 행정포탈"
      p.text-gray-500: "시스템에 로그인하세요"
...
MainPortalLayout
  Layout.Header height=64px, background=#001529, px=24px
    Left: Text strong white 18px: "해군 행정포탈"
...
Modal
  ...
  Content:
    ...
    Text 20px semibold colorError: countdown 숫자 (중앙 정렬)
```
