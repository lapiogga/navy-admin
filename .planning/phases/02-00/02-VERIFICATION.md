...
Phase 2 메인 포탈은 직접 수정 대상이 아니나, 공통 컴포넌트 강화로 포탈 UI 일관성이 향상됨.

### 포탈 영향 사항

- 서브시스템 목록/공지사항 테이블에 `navy-bordered-table` CSS 자동 적용
- 대시보드 검색 영역(있는 경우) `search-form-container` 스타일 자동 적용
- 기존 검증 결과(11/11 PASSED)는 유효. 포탈 레이아웃/세션관리/서브시스템 전환 로직은 변경 없음
