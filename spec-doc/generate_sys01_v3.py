...
LSUB  = RGBColor(0xA0, 0xD0, 0xE8)  # 부제 색상

# ═══ 치수 ═══
SW = Inches(10)
SH = Inches(5.625)
ML = Inches(0.4)       # 좌우 마진
CW = Inches(9.2)       # 콘텐츠 폭 = 10 - 0.4*2
FONT = '맑은 고딕'
...
def p1_concept(prs):
    s = _sl(prs)
    _title(s, '시스템 개념도',
           '초과근무관리체계의 전체 아키텍처 및 기술 구성 — 6개 계층', 1)

    # 6계층 데이터: (라벨, [(텍스트, fill, stroke), ...])
    layers = [
        ('사용자\n계층', [
            ('시스템관리자\n전체 관리 권한', ICE, NAVY),
            ('부대장/결재자\n결재·월말결산', ICE, NAVY),
        ...
        ('채널\n계층', [
            ...
            ('반응형 모바일 웹\n360px~', ICE, TEAL),
            ('API Gateway\nRESTful 62 endpoints', ICE, TEAL),
        ]),
        ('프론트\n엔드', [
            ('React 18 + TS 5\nVite 5 · Router 7', LBLUE, SKY),
            ('Ant Design 5.24\nProTable · ProLayout', LBLUE, SKY),
            ('Zustand 5 + TQ 5\n상태관리 · 서버캐싱', LBLUE, SKY),
        ]),
        ('백엔드\n계층', [
            ('Spring Boot 3.x\n(추후 개발) Java 17', MINT, GRN),
            ('MSW 2.12 Mock\nFaker.js 9 데이터', MINT, GRN),
            ('REST API 설계\nJSON/HTTPS 62개', MINT, GRN),
        ]),
        ('데이터\n계층', [
            ('PostgreSQL 15\n(추후) 정규화 설계', CREAM, WARN),
            ('Mock 데이터\nFaker.js 한국어', CREAM, WARN),
            ('파일 스토리지\n(추후) NAS/Object', CREAM, WARN),
        ]),
        ('외부\n연계', [
            ('GPKI/SSO 인증\n(추후 연계)', SNOW, ASH),
            ('알림톡/SMS\n(추후 연계)', SNOW, ASH),
    ...
    y0 = Inches(0.82)
    LH = Inches(0.37)       # 레이어 높이
    GAP = Inches(0.1)        # 레이어 간격
    LBL_W = Inches(0.85)    # 라벨 폭
    LBL_G = Inches(0.05)    # 라벨-콘텐츠 간격
    ...
    _tbl(s, ML, dy, CW, Inches(1.45), [
        ...
        ['• React 18.x + TypeScript 5.x\n'
         '• Ant Design 5.24 (ProTable/Layout)\n'
...
def p4_storyboard(prs):
    ...
    _title(s, '스토리보드 (화면흐름도)',
           '총 32개 화면, 7개 업무영역, 핵심 네비게이션 흐름', 4)
...
def p5_flowchart(prs):
    s = _sl(prs)
    _title(s, '업무처리 플로우차트',
           '핵심 프로세스: 초과근무 신청 → 결재 → 마감 — 4개 레인, 12단계', 5)
...
def main():
    ...
    out_path = os.path.join(OUT, 'SYS01_초과근무관리체계_분석설계_v3.pptx')
    prs.save(out_path)
