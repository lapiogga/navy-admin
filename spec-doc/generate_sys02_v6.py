#!/usr/bin/env python3
"""
SYS02 설문종합관리체계 분석설계 PPTX v6 (portrait)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
def p1_concept(prs):
    s = _sl(prs)
    _title(s, '시스템 개념도',
           '설문종합관리체계의 전체 아키텍처 및 기술 구성 — 6개 계층', 1)
    ...
    _tbl(s, ML, dy, CW, Inches(1.45), [
        ...
        ['• React 18.x + TypeScript 5.x\n'
         '• Ant Design 5.24 (ProTable/Charts)\n'
         '• Zustand 5 + TanStack Query 5\n'
         '• React Router 7 + Vite 5\n'
         '• CrudForm + SearchForm 공통\n'
         '• 7개 페이지 컴포넌트, 11개 화면',
...
def p3_usecase(prs):
    ...
    _title(s, '유스케이스 다이어그램',
           '5개 액터 x 15개 유스케이스, 31개 단위 프로세스', 3)
...
def p4_storyboard(prs):
    ...
    _title(s, '스토리보드 (화면흐름도)',
           '총 11개 화면, 3개 업무영역, 핵심 네비게이션 흐름', 4)
...
def p5_flowchart(prs):
    s = _sl(prs)
    _title(s, '업무처리 플로우차트',
           '핵심 프로세스: 설문 생성 → 승인 → 배포 → 응답 → 마감 — 4개 레인, 12단계', 5)
...
def main():
    ...
    out_path = os.path.join(OUT, 'SYS02_설문종합관리체계_분석설계_v6.pptx')
    prs.save(out_path)
