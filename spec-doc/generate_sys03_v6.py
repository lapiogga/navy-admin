#!/usr/bin/env python3
"""
SYS03 성과관리체계 분석설계 PPTX v6 (portrait)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
def p1_concept(prs):
    s = _sl(prs)
    _title(s, '시스템 개념도',
           '성과관리체계의 전체 아키텍처 및 기술 구성 — 6개 계층', 1)
...
def p3_usecase(prs):
    ...
    _title(s, '유스케이스 다이어그램',
           '5개 액터 x 20개 유스케이스, 76개 단위 프로세스', 3)
...
def p4_storyboard(prs):
    ...
    _title(s, '스토리보드 (화면흐름도)',
           '총 25개 화면, 6개 업무영역, 핵심 네비게이션 흐름', 4)
...
def p5_flowchart(prs):
    s = _sl(prs)
    _title(s, '업무처리 플로우차트',
           '핵심 프로세스: 과제등록 → 실적입력 → 승인 → 평가 — 4개 레인, 12단계', 5)
...
def main():
    ...
    out_path = os.path.join(OUT, 'SYS03_성과관리체계_분석설계_v6.pptx')
    prs.save(out_path)
