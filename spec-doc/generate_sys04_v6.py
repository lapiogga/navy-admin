#!/usr/bin/env python3
"""
SYS04 인증서발급신청체계 기능명세서 PPTX v6 (portrait)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
def p1_concept(prs):
    s = _sl(prs)
    _title(s, '시스템 개념도',
           '인증서발급신청체계의 전체 아키텍처 및 기술 구성 — 6개 계층', 1)

    layers = [
        ('사용자\n계층', [
            ('신청자 (일반사용자)\n인증서 신청·수정·회수', ICE, NAVY),
            ('승인자 (관리자)\n승인·반려·등록대장', ICE, NAVY),
        ...
        ('백엔드\n계층', [
            ...
            ('REST API 설계\nJSON/HTTPS 7개', MINT, GRN),
        ]),
        ('데이터\n계층', [
            ('PostgreSQL 15\n(추후) 정규화 설계', CREAM, WARN),
            ('Mock 데이터\nFaker.js 한국어 20건', CREAM, WARN),
...
def p3_usecase(prs):
    ...
    _title(s, '유스케이스 다이어그램',
           '3개 액터 x 10개 유스케이스, 14개 단위 프로세스', 3)
...
def p4_storyboard(prs):
    ...
    _title(s, '스토리보드 (화면흐름도)',
           '총 10개 화면, 3개 업무영역, 핵심 네비게이션 흐름', 4)
...
def p5_flowchart(prs):
    s = _sl(prs)
    _title(s, '업무처리 플로우차트',
           '핵심 프로세스: 인증서 신청 → 승인 → 등록대장 — 4개 레인, 12단계', 5)
...
def main():
    ...
    out_path = os.path.join(OUT, 'SYS04_인증서발급신청체계_분석설계_v6.pptx')
    prs.save(out_path)
