#!/usr/bin/env python3
"""
SYS05 행정규칙포탈체계 기능명세서 PPTX v6 (portrait)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
def p1_concept(prs):
    s = _sl(prs)
    _title(s, '시스템 개념도',
           '행정규칙포탈체계의 전체 아키텍처 및 기술 구성 — 6개 계층', 1)

    layers = [
        ('사용자\n계층', [
            ('규정관리자\n규정/예규/지시 등록·관리', ICE, NAVY),
            ('일반사용자 (조회자)\n규정 검색·열람·즐겨찾기', ICE, NAVY),
            ('시스템관리자\n코드·권한·시스템 관리', ICE, NAVY),
        ]),
        ('채널\n계층', [
            ('웹 브라우저\nChrome/Edge 1920×1080', ICE, TEAL),
            ('반응형 모바일 웹\n360px~', ICE, TEAL),
            ('API Gateway\nRESTful 16 endpoints', ICE, TEAL),
        ]),
        ('프론트\n엔드', [
        ...
        ('백엔드\n계층', [
            ...
            ('REST API 설계\nJSON/HTTPS 16개', MINT, GRN),
        ]),
        ('데이터\n계층', [
            ('PostgreSQL 15\n(추후) 정규화 설계', CREAM, WARN),
            ('Mock 데이터 45건\n규정15+예규22+지시8', CREAM, WARN),
...
def p2_target(prs):
    ...
    _tbl(s, ML, dy, CW, Inches(1.25), [
        ...
        ['규정관리', '부서별 분산, 최신본 혼란', '단일 DB, 문서번호 기반 관리', '최신 규정 즉시 접근', '검색 1초'],
        ['예규관리', '본부/부대 별도 관리', '2계층 통합 (본부+부대 10개)', '예규 통합 조회', '통합 100%'],
...
def p3_usecase(prs):
    ...
    _title(s, '유스케이스 다이어그램',
           '3개 액터 x 12개 유스케이스, 15개 단위 프로세스', 3)
    ...
    _tb(s, BX + Inches(0.1), y0 - Inches(0.02), Inches(5), Inches(0.18),
        '행정규칙포탈체계 (SYS05) — 바운더리', 8, True, NAVY)

    # UC 그룹
    uc_groups = [
        ('현행규정 관리 (4)', 'UC-RULE-001~004\nCRUD+검색+조직도필터+즐겨찾기', ICE, SKY),
        ('해군본부 예규 (3)', 'UC-RULE-005~007\n예규 CRUD+검색+상세조회', ICE, SKY),
        ('예하부대 예규 (1)', 'UC-RULE-008\n10개 부대 Card 그리드 조회', ICE, SKY),
        ('지시문서 관리 (3)', 'UC-RULE-009~011\n지시 CRUD+검색+상세조회', ICE, SKY),
        ('게시판 (2)', 'UC-RULE-012~013\n공지사항·질의응답', LBLUE, TEAL),
    ...
    _tbl(s, ML, dy, CW, Inches(2.1), [
        ...
        ['UC-RULE-001', '현행규정 조회', '사용자',
         'Row 6:18 레이아웃 — 좌: Tree 조직도(3뎁스), 우: SearchForm(3필드)+DataTable(9컬럼)', '상'],
        ['UC-RULE-002', '규정 등록/수정', '관리자',
         'CrudForm 7필드 Modal(규정명/문서번호/분류/소관부서/시행일/내용/첨부파일)', '상'],
        ['UC-RULE-003', '규정 상세/즐겨찾기', '사용자',
         'Descriptions(bordered) 상세 Modal + Star 즐겨찾기 토글 + 다운로드/프린트 버튼', '상'],
        ['UC-RULE-005', '해군본부 예규 관리', '관리자',
         'SearchForm(2필드)+DataTable(7컬럼)+CrudForm(5필드) Modal, CRUD 완전 지원', '상'],
        ['UC-RULE-008', '예하부대 예규 조회', '사용자',
         'Card 그리드(4열), 10개 부대 카드, 부대유형 Tag(사단/여단/단/대대), 예규 건수 표시', '중'],
        ['UC-RULE-009', '지시문서 관리', '관리자',
         'SearchForm(2필드)+DataTable(7컬럼)+CrudForm(5필드) Modal, 발령부서·발령일', '중'],
        ['UC-RULE-012', '게시판', '사용자',
         'SimpleBoardPage 재사용: 공지사항(/sys05/board/1), 질의응답(/sys05/board/2)', '하'],
...
def p4_storyboard(prs):
    ...
    _title(s, '스토리보드 (화면흐름도)',
           '총 11개 화면, 5개 업무영역, 핵심 네비게이션 흐름', 4)
    ...
    _box(s, ML + Inches(1.65), y0, Inches(2.0), Inches(0.35),
         '대시보드 / 홈\n/sys05', fill=NAVY, stk=NAVY, sz=8, b=True, fc=WHITE)

    _darr(s, ML + Inches(2.65), y0 + Inches(0.37))

    # 5개 메뉴 그룹
    groups = [
        ('1. 해군규정', '현행규정 (Tree+DataTable)', '/sys05/1/*', '1', ICE, SKY),
        ('2. 예규', '해군본부 · 예하부대', '/sys05/2/*', '2', ICE, SKY),
        ('3. 지시', '지시문서', '/sys05/3/*', '1', ICE, SKY),
        ('4. 게시판', '공지사항 · 질의응답', '/sys05/board/*', '2', LBLUE, TEAL),
        ('5. 관리자', '시스템 · 코드 · 권한 · 결재선 · 게시판', '/sys05/admin/*', '5', LBLUE, TEAL),
    ]
    ...
    _tbl(s, ML, dy, LW, Inches(0.95), [
        ...
        ['현행규정', '1', '/sys05/1/1', 'Tree+SearchForm+DataTable+Modal'],
        ['예규(본부)', '1', '/sys05/2/1', 'SearchForm+DataTable+CrudForm'],
        ['예규(부대)', '1', '/sys05/2/2', 'Card Grid(4열) + Tag'],
        ['지시문서', '1', '/sys05/3/1', 'SearchForm+DataTable+CrudForm'],
        ['게시판+관리자', '7', '/sys05/board,admin/*', 'SimpleBoardPage+AdminRoutes'],
...
def p5_flowchart(prs):
    ...
    _tbl(s, RX, dy, RW, Inches(1.42), [
        ['업무 규칙 / 예외 처리'],
        ['[업무 규칙]\n'
         'BR-RULE-001: 문서분류 4종 (법령/훈령/예규/고시)\n'
         'BR-RULE-002: 조직도 3뎁스 (본부→참모부/직할/처→부서)\n'
         'BR-RULE-003: 데이터 4종 (규정15+본부예규12+부대예규10+지시8)\n'
         'BR-RULE-004: 예하부대 10개 (작전사/함대3/잠수함/항공/교육/군수/특전/사관)\n'
         'BR-RULE-005: 부대유형 4종 Tag (사단/여단/단/대대)\n'
         'BR-RULE-006: CrudForm 7필드 CRUD (규정), 5필드 (예규/지시)\n'
         'BR-RULE-007: 즐겨찾기 useFavorites 훅 (LocalStorage)\n'
         'BR-RULE-008: 상세 Modal에 다운로드/프린트 버튼'],
        ['[예외 처리]\n'
...
def main():
    ...
    out_path = os.path.join(OUT, 'SYS05_행정규칙포탈체계_분석설계_v6.pptx')
    prs.save(out_path)
