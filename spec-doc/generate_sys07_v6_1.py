#!/usr/bin/env python3
"""
SYS07 군사자료관리체계 기능명세서 PPTX v6.1 (portrait, 4페이지)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
def p3_usecase(prs):
    ...
    _title(s, '유스케이스 다이어그램',
           '4개 액터 x 7개 UC 그룹, 40개 단위 프로세스', 2)
...
def p4_storyboard(prs):
    ...
    _title(s, '스토리보드 (화면흐름도)',
           '총 12개 메뉴, 4개 전용 화면 + 2 게시판 + 관리자, 핵심 네비게이션 흐름', 3)
...
def p5_flowchart(prs):
    s = _sl(prs)
    _title(s, '업무처리 플로우차트',
           '핵심 프로세스: 등록 → 활용(대출) → 평가심의 → 통계 — 4개 레인, 12단계', 4)
...
def main():
    ...
    out_path = os.path.join(OUT, 'SYS07_군사자료관리체계_분석설계_v6.1.pptx')
    prs.save(out_path)
