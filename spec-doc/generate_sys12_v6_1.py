#!/usr/bin/env python3
"""
SYS12 지시건의사항관리체계 기능명세서 PPTX v6.1 (portrait, 4페이지)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
def p3_usecase(prs):
    ...
    _title(s, '유스케이스 다이어그램',
           '3개 액터 x 14개 유스케이스, 32개 단위 프로세스', 2)
...
def p4_storyboard(prs):
    ...
    _title(s, '스토리보드 (화면흐름도)',
           '총 13개 화면, 4개 업무영역, 핵심 네비게이션 흐름', 3)
...
def p5_flowchart(prs):
    s = _sl(prs)
    _title(s, '업무처리 플로우차트',
           '핵심 프로세스: 지시 등록 -> 추적 -> 조치 — 4개 레인, 12단계', 4)
...
def main():
    ...
    out_path = os.path.join(OUT, 'SYS12_지시건의사항관리체계_분석설계_v6.1.pptx')
    prs.save(out_path)
