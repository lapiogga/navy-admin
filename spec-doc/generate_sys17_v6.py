#!/usr/bin/env python3
"""
SYS17 검열결과관리체계 기능명세서 PPTX v6 (portrait)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
def p1_concept(prs):
    s = _sl(prs)
    _title(s, '시스템 개념도',
           '검열결과관리체계의 전체 아키텍처 및 기술 구성 — 6개 계층', 1)

    layers = [
        ('사용자\n계층', [
            ('검열관리자\n검열계획·부대 지정·결재', ICE, NAVY),
            ('조치부서 담당자\n조치결과 입력·보고', ICE, NAVY),
            ('시스템관리자\n코드·권한·부대·로그 관리', ICE, NAVY),
        ]),
        ('채널\n계층', [
        ...
        ('프론트\n엔드', [
            ...
            ('Ant Design 5.24\nTabs·Steps·Timeline', LBLUE, SKY),
            ('Zustand 5 + TQ 5\n상태관리 · 서버캐싱', LBLUE, SKY),
        ]),
        ('백엔드\n계층', [
            ('Spring Boot 3.x\n(추후 개발) Java 17', MINT, GRN),
            ('MSW 2.12 Mock\nFaker.js 9 데이터', MINT, GRN),
            ('REST API 설계\nJSON/HTTPS 20개', MINT, GRN),
        ]),
        ('데이터\n계층', [
            ('PostgreSQL 15\n(추후) 정규화 설계', CREAM, WARN),
            ('Mock 데이터 90건\n계획10+과제30+이력50', CREAM, WARN),
            ('파일 스토리지\n첨부파일 (추후)', CREAM, WARN),
        ]),
        ('외부\n연계', [
            ('GPKI/SSO 인증\n(추후 연계)', SNOW, ASH),
    ...
    _tbl(s, ML, dy, CW, Inches(1.45), [
        ...
        ['• React 18.x + TypeScript 5.x\n'
         '• Ant Design 5.24 (Tabs/Steps/Timeline)\n'
...
def p3_usecase(prs):
    ...
    _title(s, '유스케이스 다이어그램',
           '3개 액터 x 14개 유스케이스, 25개 단위 프로세스', 3)
...
def p4_storyboard(prs):
    ...
    _title(s, '스토리보드 (화면흐름도)',
           '총 14개 화면, 3개 업무영역, 핵심 네비게이션 흐름', 4)
...
def p5_flowchart(prs):
    s = _sl(prs)
    _title(s, '업무처리 플로우차트',
           '핵심 프로세스: 부대지정 → 계획 → 과제등록 → 결재 — 4개 레인, 12단계', 5)
...
def main():
    ...
    out_path = os.path.join(OUT, 'SYS17_검열결과관리체계_분석설계_v6.pptx')
    prs.save(out_path)
