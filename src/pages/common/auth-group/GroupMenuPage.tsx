...
export function GroupMenuPage() {
  ...
  const { isLoading: permsLoading } = useQuery({
    queryKey: ['group-menus', selectedGroupId],
    queryFn: async () => {
      if (!selectedGroupId) return null
      const res = await menuPermissionApi.getGroupMenus(selectedGroupId)
  ...
  const saveMutation = useMutation({
    mutationFn: (menuPaths: string[]) =>
      menuPermissionApi.assignGroupMenus(selectedGroupId!, menuPaths),
    onSuccess: () => {
      message.success('저장되었습니다')
      queryClient.invalidateQueries({ queryKey: ['group-menus', selectedGroupId] })
    },
    onError: () => {
