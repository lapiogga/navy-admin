...
export default function RegulationListPage() {
  ...
  const handleDelete = async (id: string) => {
    await fetch(`/api/sys05/regulations/${id}`, { method: 'DELETE' })
  ...
  const handleFinish = async (values: Record<string, unknown>): Promise<boolean> => {
    ...
    try {
      ...
      if (editTarget) {
        await fetch(`/api/sys05/regulations/${editTarget.id}`, {
          method: 'PUT',
        ...
        await fetch('/api/sys05/regulations', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(formValues),
        })
