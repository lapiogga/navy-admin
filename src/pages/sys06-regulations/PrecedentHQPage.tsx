/**
 * SYS06 해병대사령부 예규 목록 페이지
 * - API 경로: /api/sys06/precedents/hq
...
async function fetchPrecedentsHQ(
  ...
  const url = new URL('/api/sys06/precedents/hq', window.location.origin)
...
export default function Sys06PrecedentHQPage() {
  ...
  const handleDelete = async (id: string) => {
    await fetch(`/api/sys06/precedents/hq/${id}`, { method: 'DELETE' })
  ...
  const handleFinish = async (values: Record<string, unknown>): Promise<boolean> => {
    ...
    try {
      ...
      if (editTarget) {
        await fetch(`/api/sys06/precedents/hq/${editTarget.id}`, {
          method: 'PUT',
        ...
        await fetch('/api/sys06/precedents/hq', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(formValues),
        })
