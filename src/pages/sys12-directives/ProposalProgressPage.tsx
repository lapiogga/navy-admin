...
async function fetchProposalProgress(): Promise<ProposalProgress> {
  const res = await apiClient.get<never, ApiResult<ProposalProgress>>('/sys12/proposals/progress')
  const data = (res as ApiResult<ProposalProgress>).data ?? (res as unknown as ProposalProgress)
  return data
}
...
export default function ProposalProgressPage() {
  const { data: progress, isLoading } = useQuery({
    queryKey: ['sys12-proposal-progress'],
    queryFn: fetchProposalProgress,
  })

  const categoryStats = progress?.categoryStats ?? []

  return (
    <PageContainer title="건의사항 처리현황">
      {/* 요약 통계 카드 */}
      <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
        <Col span={4}>
          <Card>
            <Statistic title="총 건의" value={progress?.total ?? 0} loading={isLoading} />
          </Card>
        </Col>
        <Col span={4}>
          <Card>
