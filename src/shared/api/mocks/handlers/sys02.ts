...
// Mock 카테고리 10건
const CATEGORY_NAMES = ['인사', '교육', '훈련', '복지', '보안', '급식', '시설', '장비', '의료', '체력']
let categories: SurveyCategory[] = Array.from({ length: 10 }, (_, i) => ({
  id: `cat-${i + 1}`,
  categoryName: CATEGORY_NAMES[i],
  description: `${CATEGORY_NAMES[i]} 관련 설문 카테고리`,
  sortOrder: i + 1,
...
const TEMPLATE_NAMES = [
  '표준 만족도 조사', '교육 효과 측정', '직무 환경 조사', '복지 요구도 조사', '보안 인식 조사',
  '급식 만족도 조사', '시설 이용 만족도', '장비 운용 평가', '건강 관리 설문', '체력검정 만족도',
  '부대 생활 만족도', '지휘관 리더십 평가',
]
let templates: SurveyTemplate[] = Array.from({ length: 12 }, (_, i) => ({
  id: `tmpl-${i + 1}`,
  templateName: TEMPLATE_NAMES[i],
  description: `${TEMPLATE_NAMES[i]} 템플릿`,
  categoryId: `cat-${(i % 10) + 1}`,
...
// Mock 설문 20건 (상태별 분포: draft 3, submitted 3, approved 3, rejected 2, active 5, closed 4)
const SURVEY_NAMES = [
  '2026년 상반기 부대생활 만족도 조사', '제3차 교육훈련 효과 측정', '직무 환경 개선 수요 조사',
  '2026년 복지 요구도 조사', '보안 인식 실태 점검 설문', '급식 만족도 정기 조사',
  '시설 이용 만족도 조사', '장비 운용실태 평가', '건강관리 실태조사', '체력검정 제도 만족도',
  '부대 의사소통 실태조사', '지휘관 리더십 역량 평가', '병영 문화 혁신 조사',
  '교육훈련 개선 의견 수렴', '정보보안 의식수준 측정', '인사관리 제도 만족도',
  '근무환경 안전실태 조사', '복지시설 운영 평가', '전투력 향상 방안 설문', '자기개발 지원 만족도',
]
const SURVEY_STATUS_DIST: SurveyStatus[] = [
  'draft', 'draft', 'draft',
...
let surveys: Survey[] = Array.from({ length: 20 }, (_, i) => {
  const status = SURVEY_STATUS_DIST[i]
  const targetCount = faker.number.int({ min: 30, max: 300 })
  const responseCount = status === 'active'
    ? faker.number.int({ min: 5, max: Math.floor(targetCount * 0.7) })
    : status === 'closed'
    ? faker.number.int({ min: Math.floor(targetCount * 0.5), max: targetCount })
    : 0
  return {
    id: `survey-${i + 1}`,
    surveyName: SURVEY_NAMES[i],
    ...
    targetUnits: faker.helpers.arrayElements(UNITS, { min: 1, max: 3 }),
    targetRanks: faker.helpers.arrayElements(RANKS, { min: 1, max: 4 }),
    targetCount,
    responseCount,
    questionCount: faker.number.int({ min: 3, max: 10 }),
...
// Mock 문항 (설문별 4~7개, 다양한 문항 텍스트)
const QUESTION_TEXTS: Record<QuestionType, string[]> = {
  radio: [
    '해당 분야에 대한 전반적 만족도는?',
    '현재 업무 환경에 만족하십니까?',
    '교육 프로그램의 질에 대해 어떻게 평가하십니까?',
    '제도 개선이 필요하다고 생각하십니까?',
    '지휘관의 의사소통 방식에 만족하십니까?',
    '복지 시설 이용 편의성은 어떻습니까?',
  ],
  checkbox: [
    '가장 개선이 필요한 분야를 선택해 주세요 (복수 선택)',
    '참여하고 싶은 교육 프로그램을 선택해 주세요',
    '이용하는 복지 시설을 모두 선택해 주세요',
    '중요하다고 생각하는 항목을 선택해 주세요',
  ],
  textarea: [
    '개선이 필요한 사항에 대해 자유롭게 작성해 주세요.',
    '추가 의견이나 건의사항이 있으면 작성해 주세요.',
    '가장 좋았던 점과 아쉬웠던 점을 작성해 주세요.',
  ],
  rate: [
    '전반적인 만족도를 평점으로 매겨주세요.',
    '서비스 품질을 평가해 주세요.',
    '교관/강사의 전문성을 평가해 주세요.',
  ],
}
const RADIO_OPTIONS_SETS = [
  ['매우 그렇다', '그렇다', '보통이다', '그렇지 않다', '매우 그렇지 않다'],
  ['매우 만족', '만족', '보통', '불만족', '매우 불만족'],
  ['매우 동의', '동의', '보통', '비동의', '매우 비동의'],
]
const CHECKBOX_OPTIONS_SETS = [
  ['인사관리', '교육훈련', '복지후생', '시설관리', '급식', '의료지원', '장비관리'],
  ['리더십 교육', '전문기술 교육', '체력단련', '외국어 교육', '정보화 교육', '안전교육'],
  ['체육관', '독서실', 'PC방', '매점', '휴게실', '세탁실', '식당'],
]
let questions: SurveyQuestion[] = surveys.flatMap((survey) =>
  Array.from({ length: faker.number.int({ min: 4, max: 7 }) }, (_, j) => {
    const qType = QUESTION_TYPES[j % QUESTION_TYPES.length]
    const typeTexts = QUESTION_TEXTS[qType]
    let options: string[] = []
    if (qType === 'radio') {
      options = RADIO_OPTIONS_SETS[j % RADIO_OPTIONS_SETS.length]
    } else if (qType === 'checkbox') {
      options = CHECKBOX_OPTIONS_SETS[j % CHECKBOX_OPTIONS_SETS.length]
    }
    return {
      id: `q-${survey.id}-${j + 1}`,
      surveyId: survey.id,
      questionText: typeTexts[j % typeTexts.length],
      questionType: qType,
      isRequired: j < 2,
      options,
      orderIndex: j + 1,
...
// Mock 응답 60건 (active/closed 설문에 대해 응답 분포)
const ANSWER_TEXTS = ['매우 그렇다', '그렇다', '보통이다', '그렇지 않다', '매우 그렇지 않다']
const TEXT_ANSWERS = [
  '전반적으로 만족스럽습니다.',
  '일부 개선이 필요합니다.',
  '교육 내용이 실무에 도움이 되었습니다.',
  '시설이 노후화되어 교체가 필요합니다.',
  '급식 품질이 향상되었으면 합니다.',
  '지휘관과의 소통이 원활했습니다.',
  '복지 혜택이 더 다양해졌으면 합니다.',
  '안전 교육이 더 자주 실시되었으면 합니다.',
  '장비 정비 주기를 단축해 주세요.',
  '체력단련 시간이 더 필요합니다.',
]
let responses: SurveyResponse[] = Array.from({ length: 60 }, (_, i) => ({
  id: `resp-${i + 1}`,
...
let targets: SurveyTarget[] = Array.from({ length: 80 }, (_, i) => ({
  ...
  isResponded: i % 3 === 0,
  respondedAt: i % 3 === 0 ? faker.date.recent({ days: 14 }).toISOString().split('T')[0] : undefined,
}))
...
export const sys02Handlers = [
  ...
  http.get('/api/sys02/surveys', ({ request }) => {
    ...
    if (keyword) {
      filtered = filtered.filter((s) =>
        s.surveyName.includes(keyword) || s.description.includes(keyword)
      )
    }
  ...
  http.get('/api/sys02/surveys/:id/results', ({ params }) => {
    const survey = surveys.find((s) => s.id === params.id)
    const surveyQuestions = questions.filter((q) => q.surveyId === params.id)
    const surveyResponses = responses.filter((r) => r.surveyId === params.id)
    const result = {
      survey,
      totalTarget: survey?.targetCount || 0,
      totalResponse: survey?.responseCount || surveyResponses.length,
      ...
      questionResults: surveyQuestions.map((q) => ({
        question: q,
        optionCounts: q.options.map((opt) => {
          const count = faker.number.int({ min: 2, max: 30 })
          return {
            option: opt,
            count,
            ratio: Math.round((count / Math.max(survey?.responseCount || 1, 1)) * 100),
          }
        }),
        textAnswers: q.questionType === 'textarea'
          ? surveyResponses
              .filter((r) => r.questionId === q.id)
              .map((r) => r.answer as string)
              .concat(['전반적으로 만족합니다.', '개선이 필요한 부분이 있습니다.', '좋은 설문이었습니다.'])
          : [],
        averageRating: q.questionType === 'rate' ? faker.number.float({ min: 2.5, max: 4.8, fractionDigits: 1 }) : undefined,
        ratingDistribution: q.questionType === 'rate' ? [
          { score: 1, count: faker.number.int({ min: 0, max: 5 }) },
          { score: 2, count: faker.number.int({ min: 2, max: 8 }) },
          { score: 3, count: faker.number.int({ min: 5, max: 15 }) },
          { score: 4, count: faker.number.int({ min: 8, max: 25 }) },
